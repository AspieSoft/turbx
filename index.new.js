...
function goCompilerSend(opt, data, value){
  if(value){
    data += '=' + value;
  }
  goCompiler.stdin.write(opt + ':' + data + '\n');
}
...
setTimeout(function(){
  goCompilerSend('test', '{"test": 1}:../test/views/index.xhtml');
}, 100)
...
function getTemplateFile(path, opts, cb, component = false){
  ...
  taskQueue(path, async (next) => {
    ...
    const idToken = randomToken(64);
    goCompilerSend(idToken, ':'+filePath);

    let loops = 1000000;
    while(loops-- > 0){
      if(!goCompiledResults[idToken]){
        await sleep(10);
      }
      break;
    }

    let res = goCompiledResults[idToken];
    delete goCompiledResults[idToken];
    ...
    try {
      /* res = JSON.parse(res.replace(/([0-9]+)/g, '$1,').replace(/,\s*]$/, ']'));
    ...
    localCache.set('template_file_cache:' + path, res, {expire: opts.cache || OPTS.cache || '2h', listen: [filePath]});
    next();
    cb(res);
  });
}
...
function engine(path, opts, cb){
  ...
  getTemplateFile(path, opts, async (data) => {
    if(data === emptyRes){
      return cb('', ''); //temp: suppress express error from log
      return cb(new Error('Turbx File Not Found!'), '');
    }
...
function setOpts(opts){
  ...
  for(let key in OPTS){
    ...
    if(typeof OPTS[key] === 'function'){
      goCompilerSend('set', key, 'true')
    }else if(typeof OPTS[key] === 'object'){
      goCompilerSend('set', key, JSON.stringify(OPTS[key]))
    }else if(typeof OPTS[key] === 'string' && OPTS[key].match(/^[0-9]+(\.[0-9]+|)[a-z]{0,3}$/)){
      let opt = toTimeMillis(OPTS[key]);
      if(!Number.isNaN(opt)){
        goCompilerSend('set', key, opt.toString())
      }else{
        goCompilerSend('set', key, OPTS[key].toString())
      }
    }else{
      goCompilerSend('set', key, OPTS[key].toString())
    }
